...
for tag in tags:
    ...
    subprocess.run(f"java -jar /usr/local/widoco/widoco.jar -ontFile copy/{onto_name}.ttl -import copy/{onto_name}.ttl -outFolder out/{tag.name[1:]} -rewriteAll -getOntologyMetadata -lang de-en -saveConfig out/config -webVowl -noPlaceHolderText -htaccess", shell=True)
    if tag == tags[-1]:
